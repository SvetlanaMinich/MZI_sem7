...
def S_transformation(a):
    if isinstance(a, int):
        byte_array = bytearray(a.to_bytes((a.bit_length() + 7) // 8, 'big'))
    else:
...
def L_transformation(a):
    if isinstance(a, int):
        input_vector = a & 0xFFFFFFFFFFFFFFFF
    else:
        input_vector = int.from_bytes(a, 'big') & 0xFFFFFFFFFFFFFFFF
    
    result = 0
    
    for i in range(64):
        # Получаем i-ю строку матрицы A из hex-представления
        matrix_row_hex = A_MATRIX_HEX[i]
        matrix_row = int(matrix_row_hex, 16)
        
        # Вычисляем скалярное произведение i-й строки матрицы с входным вектором
        # Используем XOR вместо сложения в поле GF(2)
        dot_product = 0
        for j in range(64):
            # Проверяем j-й бит в строке матрицы и входном векторе
            if (matrix_row >> (63 - j)) & 1 and (input_vector >> (63 - j)) & 1:
                dot_product ^= 1
        
        # Устанавливаем i-й бит результата
        result |= (dot_product << (63 - i))
    
    return result


# ------ COMPRESSION GOST 34.11-2018
C = ["b1085bda1ecadae9ebcb2f81c0657c1f2f6a76432e45d016714eb88d7585c4fc4b7се09192676901a2422a08a460d31505767436cc744d23dd806559f2a64507",
...
def hash_512(m, output=512):
    ...
    if isinstance(m, str):
        ...
        m = bytearray(m.to_bytes((m.bit_length() + 7) // 8, 'big'))
    ...
    m = int.from_bytes(m_with_padding, 'big')
